...
public class KettleScreen extends AbstractContainerScreen<KettleContainer> {
    ...
    public void render(GuiGraphics guiGraphics, final int mouseX, final int mouseY, float partialTicks) {
        ...
        this.renderWaterBarIndicatorTooltip(guiGraphics, mouseX, mouseY);
    }
    ...
    private void renderWaterBarIndicatorTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    ...
    protected void renderMealDisplayTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        //guiGraphics.blit(ms, this.leftPos, this.topPos, 0, 0, this.imageWidth, this.imageHeight);
        ...
        guiGraphics.blit(BACKGROUND_TEXTURE, this.leftPos + PROGRESS_ARROW.x, this.topPos + PROGRESS_ARROW.y, 176, 15, l + 1, PROGRESS_ARROW.height);

        // Render water bar
        if (this.menu.waterLevel() == 1) {
            guiGraphics.blit(BACKGROUND_TEXTURE, this.leftPos + WATER_BAR1.x, this.topPos + WATER_BAR1.y, 176, 53, WATER_BAR1.width, WATER_BAR1.height);
        }
        if (this.menu.waterLevel() == 2) {
            guiGraphics.blit(BACKGROUND_TEXTURE, this.leftPos + WATER_BAR1.x, this.topPos + WATER_BAR1.y, 176, 53, WATER_BAR1.width, WATER_BAR1.height);
            guiGraphics.blit(BACKGROUND_TEXTURE, this.leftPos + WATER_BAR2.x, this.topPos + WATER_BAR2.y, 176, 43, WATER_BAR2.width, WATER_BAR2.height);
        }
        if (this.menu.waterLevel() == 3) {
            guiGraphics.blit(BACKGROUND_TEXTURE, this.leftPos + WATER_BAR1.x, this.topPos + WATER_BAR1.y, 176, 53, WATER_BAR1.width, WATER_BAR1.height);
            guiGraphics.blit(BACKGROUND_TEXTURE, this.leftPos + WATER_BAR2.x, this.topPos + WATER_BAR2.y, 176, 43, WATER_BAR2.width, WATER_BAR2.height);
            guiGraphics.blit(BACKGROUND_TEXTURE, this.leftPos + WATER_BAR3.x, this.topPos + WATER_BAR3.y, 176, 32, WATER_BAR3.width, WATER_BAR3.height);
        }
    }
}
